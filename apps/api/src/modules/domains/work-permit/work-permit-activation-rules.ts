 ...
 */
export function assertGasTestPassedIfRequired(requiresGasTest: boolean, results: GasTestActivationCandidate[]): void {
  if (!requiresGasTest) return;
 ...
 */
export function assertLotoVerifiedIfRequired(requiresLoto: boolean, records: LotoActivationCandidate[]): void {
  if (!requiresLoto) return;
