...
@Injectable()
export class WorkPermitWorkflowBootstrapService {
  ...
  private async findRoleOrThrow(tx: Prisma.TransactionClient, roleCode: string) {
    ...
    if (!role) {
      throw new NotFoundException(
        `Role sistem "${roleCode}" tidak ditemukan — RBAC baseline (seed-rbac-baseline.ts) wajib dijalankan sebelum modul Work Permit bisa membuat workflow_definitions default.`,
      );
    }
    return role;
  }
   ...
   * permit induknya).
   */
  async ensureExtensionWorkflowDefinition(tx: Prisma.TransactionClient): Promise<WorkflowDefinition> {
    const tenantId = requireTenantId();
