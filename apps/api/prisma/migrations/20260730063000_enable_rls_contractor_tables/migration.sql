...
DO $$ BEGIN IF EXISTS (SELECT 1 FROM pg_roles WHERE rolname = 'qhse_app') THEN EXECUTE $sql$GRANT SELECT, INSERT, UPDATE, DELETE ON "contractors" TO qhse_app$sql$; END IF; END $$;
...
CREATE POLICY tenant_isolation_policy ON "contractor_prequalification"
  USING (tenant_id = current_setting('app.current_tenant_id', true)::uuid);
DO $$ BEGIN IF EXISTS (SELECT 1 FROM pg_roles WHERE rolname = 'qhse_app') THEN EXECUTE $sql$GRANT SELECT, INSERT, UPDATE, DELETE ON "contractor_prequalification" TO qhse_app$sql$; END IF; END $$;
...
CREATE POLICY tenant_isolation_policy ON "contractor_prequalification_documents"
  USING (tenant_id = current_setting('app.current_tenant_id', true)::uuid);
DO $$ BEGIN IF EXISTS (SELECT 1 FROM pg_roles WHERE rolname = 'qhse_app') THEN EXECUTE $sql$GRANT SELECT, INSERT, UPDATE, DELETE ON "contractor_prequalification_documents" TO qhse_app$sql$; END IF; END $$;
...
CREATE POLICY tenant_isolation_policy ON "contractor_document_compliance"
  USING (tenant_id = current_setting('app.current_tenant_id', true)::uuid);
DO $$ BEGIN IF EXISTS (SELECT 1 FROM pg_roles WHERE rolname = 'qhse_app') THEN EXECUTE $sql$GRANT SELECT, INSERT, UPDATE, DELETE ON "contractor_document_compliance" TO qhse_app$sql$; END IF; END $$;
...
CREATE POLICY tenant_isolation_policy ON "contractor_workers"
  USING (tenant_id = current_setting('app.current_tenant_id', true)::uuid);
DO $$ BEGIN IF EXISTS (SELECT 1 FROM pg_roles WHERE rolname = 'qhse_app') THEN EXECUTE $sql$GRANT SELECT, INSERT, UPDATE, DELETE ON "contractor_workers" TO qhse_app$sql$; END IF; END $$;
...
CREATE POLICY tenant_isolation_policy ON "contractor_project_assignments"
  USING (tenant_id = current_setting('app.current_tenant_id', true)::uuid);
DO $$ BEGIN IF EXISTS (SELECT 1 FROM pg_roles WHERE rolname = 'qhse_app') THEN EXECUTE $sql$GRANT SELECT, INSERT, UPDATE, DELETE ON "contractor_project_assignments" TO qhse_app$sql$; END IF; END $$;
...
CREATE POLICY tenant_isolation_policy ON "contractor_performance_evaluations"
  USING (tenant_id = current_setting('app.current_tenant_id', true)::uuid);
DO $$ BEGIN IF EXISTS (SELECT 1 FROM pg_roles WHERE rolname = 'qhse_app') THEN EXECUTE $sql$GRANT SELECT, INSERT, UPDATE, DELETE ON "contractor_performance_evaluations" TO qhse_app$sql$; END IF; END $$;
