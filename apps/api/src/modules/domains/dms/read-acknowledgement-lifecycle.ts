...
export function validateAcknowledgementTransition(
  from: ReadAcknowledgementStatus,
  to: ReadAcknowledgementStatus,
  method: AcknowledgementMethod | null,
): void {
  if ((from === "PENDING" || from === "OVERDUE") && to === "VIEWED") return;
  if ((from === "VIEWED" || from === "OVERDUE") && to === "ACKNOWLEDGED") return;
  if (from === "PENDING" && to === "ACKNOWLEDGED" && method === "E_SIGNATURE") return;
  if ((from === "PENDING" || from === "VIEWED") && to === "OVERDUE") return;
