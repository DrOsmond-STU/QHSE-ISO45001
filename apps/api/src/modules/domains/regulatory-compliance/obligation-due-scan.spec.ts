...
function candidate(overrides: Partial<ObligationDueCandidate>): ObligationDueCandidate {
  return {
    obligationId: "obl-1",
    nextDueDate: null,
    status: "ACTIVE",
...
describe("findObligationsDueForReminder", () => {
  it("includes ACTIVE obligation within 30-day window", () => {
    ...
    expect(findObligationsDueForReminder([c], NOW)).toEqual([c]);
  });
  ...
  it("excludes obligation outside the 30-day window", () => {
    ...
    expect(findObligationsDueForReminder([c], NOW)).toEqual([]);
  });
  ...
  it.each(["INACTIVE", "RETIRED"] as const)("excludes obligation with status %s", (status) => {
...
describe("findOverdueObligations", () => {
  it("includes ACTIVE obligation past next_due_date", () => {
    ...
    expect(findOverdueObligations([c], NOW)).toEqual([c]);
  });

  it("excludes obligation due exactly today (not yet past)", () => {
    const c = candidate({ nextDueDate: NOW });
    expect(findOverdueObligations([c], NOW)).toEqual([]);
  });
