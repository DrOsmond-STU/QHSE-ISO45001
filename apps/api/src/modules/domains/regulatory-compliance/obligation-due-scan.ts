 ...
 */
export function findObligationsDueForReminder(candidates: ObligationDueCandidate[], now: Date): ObligationDueCandidate[] {
  return candidates.filter((c) => {
    if (c.status !== "ACTIVE") return false;
    if (c.nextDueDate === null) return false;
    if (c.dueReminderSentAt !== null) return false;
 ...
 */
export function findOverdueObligations(candidates: ObligationDueCandidate[], now: Date): ObligationDueCandidate[] {
  return candidates.filter((c) => {
    if (c.status !== "ACTIVE") return false;
    if (c.nextDueDate === null) return false;
