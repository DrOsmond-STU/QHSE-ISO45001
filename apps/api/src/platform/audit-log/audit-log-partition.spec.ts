import { addMonthsUtc, partitionSpecForMonth } from "./audit-log-partition";

describe("partitionSpecForMonth", () => {
  it("Juli 2026 -> nama partisi + rentang bulan kalender penuh", () => {
    const spec = partitionSpecForMonth(new Date(Date.UTC(2026, 6, 24, 15, 30)));
    expect(spec).toEqual({
  ...
  it("Desember -> rentang menyeberang ke Januari tahun berikutnya", () => {
    const spec = partitionSpecForMonth(new Date(Date.UTC(2026, 11, 5)));
    expect(spec).toEqual({
  ...
  it("bulan 1 digit di-zero-pad (m07, bukan m7)", () => {
    const spec = partitionSpecForMonth(new Date(Date.UTC(2026, 0, 1)));
...
describe("addMonthsUtc", () => {
  it("menambah 1 bulan dalam tahun yang sama", () => {
    const result = addMonthsUtc(new Date(Date.UTC(2026, 6, 24)), 1);
    expect(result.getUTCFullYear()).toBe(2026);
    expect(result.getUTCMonth()).toBe(7); // Agustus (0-based)
  });

  it("menambah lintas pergantian tahun", () => {
    const result = addMonthsUtc(new Date(Date.UTC(2026, 11, 24)), 1);
