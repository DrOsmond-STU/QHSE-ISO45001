...
const MANDATORY_REGULATORY_REPORT_CLASSIFICATIONS: ReadonlySet<IncidentClassification> = new Set([
  "FATALITY",
  "LOST_TIME_INJURY",
...
export const DEFAULT_REGULATORY_REPORT_TENGGAT_HOURS = 48;

export function shouldAutoCreateRegulatoryReport(classification: IncidentClassification): boolean {
  return MANDATORY_REGULATORY_REPORT_CLASSIFICATIONS.has(classification);
}

export function computeRequiredByDate(incidentDatetime: Date): Date {
  return new Date(incidentDatetime.getTime() + DEFAULT_REGULATORY_REPORT_TENGGAT_HOURS * 60 * 60 * 1000);
}
...
export function assertNoPendingOrOverdueRegulatoryReports(reports: { status: IncidentRegulatoryReportStatus }[]): void {
  const blocking = reports.filter((r) => r.status === "PENDING" || r.status === "OVERDUE");
  if (blocking.length > 0) {
    throw new Error(
      `incident_reports tidak dapat CLOSED — masih ada ${blocking.length} incident_regulatory_reports berstatus PENDING/OVERDUE (BR-09).`,
    );
  }
}
