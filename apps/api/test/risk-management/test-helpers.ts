...
/** company_id/site_id pada hazard_register PUNYA FK sungguhan — test yang
 * perlu mengisi siteId wajib pakai baris nyata, bukan randomUUID(). */
...
/** roleCode ROLE_CODES key (mis. "SUPERVISOR"/"HSE_MANAGER"/"COMPANY_ADMIN")
 * — scopeType TENANT/scopeId null (pola sama regulatory-compliance
 * test-helpers.ts) krn ApproverResolutionService.resolveApprovers()
 * ROLE_IN_SCOPE MASIH tenant-wide saja (lihat banner comment method itu),
 * belum mempersempit ke site/department entity yang di-approve. */
...
export const ROLE_CODES_RISK = ROLE_CODES;
