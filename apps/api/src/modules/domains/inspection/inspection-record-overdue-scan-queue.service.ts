...
import {
  INSPECTION_RECORD_OVERDUE_SCAN_CRON,
  INSPECTION_RECORD_OVERDUE_SCAN_JOB_NAME,
  INSPECTION_RECORD_OVERDUE_SCAN_QUEUE,
} from "./inspection-record-overdue-scan.constants";
...
@Injectable()
export class InspectionRecordOverdueScanQueueService implements OnApplicationBootstrap, OnModuleDestroy {
  private readonly logger = new Logger(InspectionRecordOverdueScanQueueService.name);
  ...
  constructor() {
    ...
    this.queue = new Queue(INSPECTION_RECORD_OVERDUE_SCAN_QUEUE, { connection: this.connection });
  }
  ...
  async onApplicationBootstrap(): Promise<void> {
    ...
    await this.queue.add(
      ...
      {},
      {
        repeat: { pattern: INSPECTION_RECORD_OVERDUE_SCAN_CRON },
        jobId: INSPECTION_RECORD_OVERDUE_SCAN_JOB_NAME,
        removeOnComplete: true,
        removeOnFail: 50,
      },
    );
    this.logger.log(`Repeatable job "${INSPECTION_RECORD_OVERDUE_SCAN_QUEUE}" terdaftar (cron: ${INSPECTION_RECORD_OVERDUE_SCAN_CRON}).`);
  }
