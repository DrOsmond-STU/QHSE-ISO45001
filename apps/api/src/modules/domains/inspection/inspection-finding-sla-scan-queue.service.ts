...
import {
  INSPECTION_FINDING_SLA_SCAN_CRON,
  INSPECTION_FINDING_SLA_SCAN_JOB_NAME,
  INSPECTION_FINDING_SLA_SCAN_QUEUE,
} from "./inspection-finding-sla-scan.constants";
...
@Injectable()
export class InspectionFindingSlaScanQueueService implements OnApplicationBootstrap, OnModuleDestroy {
  private readonly logger = new Logger(InspectionFindingSlaScanQueueService.name);
  ...
  constructor() {
    ...
    this.queue = new Queue(INSPECTION_FINDING_SLA_SCAN_QUEUE, { connection: this.connection });
  }
  ...
  async onApplicationBootstrap(): Promise<void> {
    ...
    await this.queue.add(
      ...
      {},
      {
        repeat: { pattern: INSPECTION_FINDING_SLA_SCAN_CRON },
        jobId: INSPECTION_FINDING_SLA_SCAN_JOB_NAME,
        removeOnComplete: true,
        removeOnFail: 50,
      },
    );
    this.logger.log(`Repeatable job "${INSPECTION_FINDING_SLA_SCAN_QUEUE}" terdaftar (cron: ${INSPECTION_FINDING_SLA_SCAN_CRON}).`);
  }
