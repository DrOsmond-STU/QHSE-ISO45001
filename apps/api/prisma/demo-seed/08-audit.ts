// Data demo Audit Management (Modul 09, task 4.1) — 3 audit_program lintas
// status (2-stage APPROVED dgn plan item->audit instance CLOSED penuh,
// REJECTED->DRAFT, DRAFT belum diajukan), 2 audit (CLOSED penuh dgn 2
// temuan+CAPA otomatis+report 2-stage CONTEXT_USER, ad-hoc IN_PROGRESS dgn
// BR-01 competency warning krn Lead Auditor TANPA sertifikat terdaftar),
// BR-07 independensi auditor (anggota tim dipilih SENGAJA beda departemen
// dari auditee scope — lihat catatan departemen di bawah), 2 scan job.
// findingService.create() utk MAJOR_NC/MINOR_NC memicu
// AuditFindingCapaTriggerListener (Modul 10, task 215) OTOMATIS membuat
// capa_register + men-CAPA_LINKED-kan finding — TIDAK perlu linkCapaRegister()
// manual, cukup jeda ~350ms (pola sama listener async lain) sebelum
// verify()/close().
import { INestApplicationContext } from "@nestjs/common";
import { PrismaClient } from "@prisma/client";
import { AuditAuditeeScopeService } from "../../src/modules/domains/audit/audit-auditee-scope.service";
import { AuditChecklistService } from "../../src/modules/domains/audit/audit-checklist.service";
...
import { AuditTypeService } from "../../src/modules/domains/audit/audit-type.service";
...
export async function seedAudit(app: INestApplicationContext, adminPrisma: PrismaClient, ctx: DemoContext): Promise<void> {
  const typeService = app.get(AuditTypeService);
  const checklistService = app.get(AuditChecklistService);
  const programService = app.get(AuditProgramService);
  const auditService = app.get(AuditService);
  const teamMemberService = app.get(AuditTeamMemberService);
  const auditeeScopeService = app.get(AuditAuditeeScopeService);
  const findingService = app.get(AuditFindingService);
  const competencyService = app.get(AuditorCompetencyRecordService);
  const reportService = app.get(AuditReportService);
  const competencyExpiryScanService = app.get(AuditorCompetencyExpiryScanService);
  const findingClosureDueScanService = app.get(AuditFindingClosureDueScanService);

  const hseManager = actor(ctx, "HSE_MANAGER");
  const companyAdmin = actor(ctx, "COMPANY_ADMIN");
  const leadAuditor = actor(ctx, "AUDITOR_INTERNAL"); // Maria — departmentId NULL (selalu independen).
  const externalAuditor = actor(ctx, "AUDITOR_EXTERNAL"); // Robert — TANPA competency record (demo BR-01).
  ...
  const auditorBalikpapan = actors(ctx, "HSE_OFFICER")[1];
  ...
  const checklist1 = await run(hseManager.id, () =>
  ...
  const program1 = await run(hseManager.id, () =>
  ...
  await approveAllPendingStages(app, adminPrisma, ctx.tenantId, program1Submitted.workflowInstanceId!);
  ...
  const audit1 = await run(hseManager.id, () =>
    auditService.createFromPlanItem(planItemCepu.id, {
  ...
  await run(hseManager.id, () => auditService.recordOpeningMeeting(audit1.id, { datetime: daysAgo(20), notes: "Dihadiri Kepala Departemen Operasi & tim produksi" }));

  const finding1a = await run(leadAuditor.id, () =>
  ...
  const finding1b = await run(leadAuditor.id, () =>
    findingService.create({
      auditId: audit1.id,
      findingNumber: "F-02",
  ...
  await run(hseManager.id, () => auditService.recordClosingMeeting(audit1.id, { datetime: daysAgo(18), notes: "Temuan dipaparkan, auditee sepakat menindaklanjuti" }));

  const report1 = await run(leadAuditor.id, () => reportService.create(audit1.id));
  await run(leadAuditor.id, () =>
    reportService.updateContent(report1.id, {
  ...
  await approveAllPendingStages(app, adminPrisma, ctx.tenantId, report1Submitted.workflowInstanceId!);
  ...
  const audit2 = await run(hseManager.id, () =>
  ...
  const finding2 = await run(externalAuditor.id, () =>
    findingService.create({
      auditId: audit2.id,
      findingNumber: "F-01",
  ...
  await adminPrisma.auditFinding.update({ where: { id: finding2.id }, data: { targetClosureDate: daysFromNow(3) } });

  // 4. Program REJECTED -> kembali DRAFT.
  const program2 = await run(hseManager.id, () =>
    programService.create({ companyId: ctx.companyId, programYear: 2027, name: "Program Audit Tambahan (Draft Awal)", planItems: [] }),
  );
  ...
  await actOnNextPendingTask(
    ...
    adminPrisma,
    ctx.tenantId,
    program2Submitted.workflowInstanceId!,
    "REJECT",
    "Objective program belum jelas, mohon dilengkapi sebelum diajukan ulang",
  );
  ...
  console.log(
    "  Audit Management: 3 program (2027 APPROVED+2 plan item, REJECTED->DRAFT, DRAFT 2028), 2 audit (Cepu CLOSED penuh+2 temuan+CAPA otomatis+report 2-stage, Balikpapan IN_PROGRESS+BR-01 warning), competency EXPIRED via scan.",
  );
}
