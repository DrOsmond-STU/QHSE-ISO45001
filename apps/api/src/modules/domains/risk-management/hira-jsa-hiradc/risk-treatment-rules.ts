 ...
 */
export function assertAcceptRequiresTopManagementApproval(
  treatmentStrategy: RiskTreatmentStrategy,
  sourceRequiresEscalation: boolean,
  topManagementApprovedBy: string | null,
): void {
  if (treatmentStrategy === "ACCEPT" && sourceRequiresEscalation && topManagementApprovedBy === null) {
    throw new Error(
      "risk_treatment_plans dengan treatment_strategy=ACCEPT pada risiko yang butuh eskalasi wajib topManagementApprovedBy terisi (BR-06).",
    );
  }
}
