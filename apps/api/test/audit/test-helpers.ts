...
/** site_id/company_id/branch_id pada audit_programs/audits PUNYA FK
 * sungguhan — test yang perlu mengisinya wajib pakai baris nyata, bukan
 * randomUUID(). Pola PERSIS test/emergency-response|incident|inspection/test-helpers.ts. */
...
export async function seedDepartment(tenantId: string, siteFixture: { companyId: string; branchId: string; siteId: string }, actorUserId: string, label = "Fixture Dept") {
  return adminPrisma.department.create({
    data: {
      tenantId,
      companyId: siteFixture.companyId,
      branchId: siteFixture.branchId,
      siteId: siteFixture.siteId,
      departmentCode: `DPT-${randomUUID().slice(0, 8)}`,
      name: label,
