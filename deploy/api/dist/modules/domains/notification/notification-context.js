...
function requireActorUserId() {
    ...
    if (!userId) {
        throw new Error("Actor user_id tidak ditemukan di context — operasi notification ditolak (fail closed).");
    }
    return userId;
}
function requireTenantId() {
    ...
    if (!tenantId) {
        throw new Error("Tenant context tidak ditemukan — operasi notification ditolak (fail closed).");
    }
    return tenantId;
}
