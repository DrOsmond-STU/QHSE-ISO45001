...
import { MCU_REMINDER_SCAN_CRON, MCU_REMINDER_SCAN_JOB_NAME, MCU_REMINDER_SCAN_QUEUE } from "./mcu-reminder-scan.constants";
...
@Injectable()
export class McuReminderScanQueueService implements OnApplicationBootstrap, OnModuleDestroy {
  private readonly logger = new Logger(McuReminderScanQueueService.name);
  ...
  constructor() {
    ...
    this.queue = new Queue(MCU_REMINDER_SCAN_QUEUE, { connection: this.connection });
  }

  async onApplicationBootstrap(): Promise<void> {
    if (!this.queue) return;
    await this.queue.add(
      MCU_REMINDER_SCAN_JOB_NAME,
      {},
      {
        repeat: { pattern: MCU_REMINDER_SCAN_CRON },
        jobId: MCU_REMINDER_SCAN_JOB_NAME,
        removeOnComplete: true,
        removeOnFail: 50,
      },
    );
    this.logger.log(`Repeatable job "${MCU_REMINDER_SCAN_QUEUE}" terdaftar (cron: ${MCU_REMINDER_SCAN_CRON}).`);
  }
