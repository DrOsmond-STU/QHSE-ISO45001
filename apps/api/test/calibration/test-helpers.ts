...
/** site_id/company_id/branch_id py FK sungguhan — pola PERSIS test/asset-equipment/test-helpers.ts. */
...
/** Modul 16 "bergantung penuh" pada assets (Modul 15) — setiap calibration_item butuh 1 asset nyata. */
export async function seedAsset(
  tenantId: string,
  siteId: string,
  actorUserId: string,
  overrides: { isSafetyCritical?: boolean } = {},
): Promise<{ assetId: string; assetCategoryId: string }> {
  const category = await adminPrisma.assetCategory.create({
    data: { tenantId, categoryName: `Kategori ${randomUUID().slice(0, 8)}`, createdBy: actorUserId, updatedBy: actorUserId },
  });
  const asset = await adminPrisma.asset.create({
    data: {
      tenantId,
      siteId,
      assetCategoryId: category.id,
      assetCode: `AST-${randomUUID().slice(0, 8)}`,
      assetName: "Pressure Gauge Fixture",
      isSafetyCritical: overrides.isSafetyCritical ?? false,
      createdBy: actorUserId,
      updatedBy: actorUserId,
    },
  });
  return { assetId: asset.id, assetCategoryId: category.id };
}
