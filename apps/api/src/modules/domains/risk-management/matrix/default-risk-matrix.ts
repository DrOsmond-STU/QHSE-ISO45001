...
export interface DefaultMatrixCell {
  ...
  // Task 3.2 — TRUE utk band EXTREME, dipakai HIRA (percabangan workflow)/
  // BR-06 (accept gate) — lihat banner comment risk_matrix_cells.requiresEscalation.
  requiresEscalation: boolean;
}

function resolveBand(score: number): { level: string; color: string; action: string; requiresEscalation: boolean } {
  if (score >= 16) return { level: "EXTREME", color: "#F44336", action: "Wajib tindakan segera & approval Direksi", requiresEscalation: true };
  if (score >= 10) return { level: "HIGH", color: "#FF9800", action: "Wajib tindakan pengendalian tambahan sebelum pekerjaan dimulai", requiresEscalation: false };
  if (score >= 5) return { level: "MEDIUM", color: "#FFC107", action: "Pantau & tinjau efektivitas kontrol yang ada", requiresEscalation: false };
  return { level: "LOW", color: "#4CAF50", action: "Dapat diterima dengan kontrol rutin", requiresEscalation: false };
}

export function buildDefaultRiskMatrixLevels(): DefaultMatrixLevel[] {
  const levels: DefaultMatrixLevel[] = [];
...
export function buildDefaultRiskMatrixCells(): DefaultMatrixCell[] {
  const cells: DefaultMatrixCell[] = [];
