...
function toNcrDetectionStage(inspectionType) {
 ...
 */
let QualityInspectionService = class QualityInspectionService {
    prisma;
    numberingService;
    bootstrapService;
    ...
    ncrRecordService;
    notificationService;
    constructor(prisma, numberingService, bootstrapService, workflowEngineService, ncrRecordService, notificationService) {
        this.prisma = prisma;
    ...
    async create(input) {
        const inspectorId = (0, quality_context_1.requireActorUserId)();
        ...
        return this.prisma.withRls((tx) => tx.qualityInspection.create({
            data: {
                tenantId,
                siteId: input.siteId,
    ...
    async recordParameterResult(parameterId, measuredValue, result) {
        const updatedBy = (0, quality_context_1.requireActorUserId)();
        await this.prisma.withRls((tx) => tx.qualityInspectionParameter.update({ where: { id: parameterId }, data: { measuredValue, result, updatedBy } }));
    }
    /**
     ...
     */
    async submit(qualityInspectionId) {
        const updatedBy = (0, quality_context_1.requireActorUserId)();
        const inspection = await this.prisma.withRls((tx) => tx.qualityInspection.findUniqueOrThrow({ where: { id: qualityInspectionId }, include: { parameters: true } }));
        (0, inspection_rules_1.validateInspectionStatusTransition)(inspection.status, "SUBMITTED");
        const resultStatus = inspection.parameters.some((p) => p.result === "FAIL") ? "FAIL" : "PASS";
        let autoGeneratedNcrId = null;
        if ((0, inspection_rules_1.shouldAutoCreateNcr)(resultStatus, inspection.inspectionType)) {
            ...
            const departmentHeads = await this.prisma.withRls((tx) => tx.user.findMany({
        ...
        return this.prisma.withRls((tx) => tx.qualityInspection.update({
     ...
     */
    async review(qualityInspectionId, overallDisposition) {
        ...
        (0, inspection_rules_1.validateInspectionStatusTransition)(inspection.status, "REVIEWED");
        let deviationWorkflowInstanceId;
        if (overallDisposition === "USE_AS_IS_DEVIATION") {
            ...
            const instance = await this.workflowEngineService.startInstance(INSPECTION_DEVIATION_WORKFLOW_ENTITY_TYPE, qualityInspectionId, definition.id, {});
    ...
    async close(qualityInspectionId) {
        const updatedBy = (0, quality_context_1.requireActorUserId)();
        const inspection = await this.prisma.withRls((tx) => tx.qualityInspection.findUniqueOrThrow({ where: { id: qualityInspectionId } }));
        (0, inspection_rules_1.validateInspectionStatusTransition)(inspection.status, "CLOSED");
        let deviationApproved = false;
        if (inspection.deviationWorkflowInstanceId) {
            const instance = await this.prisma.withRls((tx) => tx.workflowInstance.findUniqueOrThrow({ where: { id: inspection.deviationWorkflowInstanceId } }));
            deviationApproved = instance.status === "APPROVED";
        }
        (0, inspection_rules_1.assertDeviationApprovedBeforeClose)(inspection.overallDisposition, deviationApproved);
        return this.prisma.withRls((tx) => tx.qualityInspection.update({ where: { id: qualityInspectionId }, data: { status: "CLOSED", updatedBy } }));
    }
    async getById(qualityInspectionId) {
        return this.prisma.withRls((tx) => tx.qualityInspection.findUniqueOrThrow({ where: { id: qualityInspectionId }, include: { parameters: true } }));
    }
...
exports.QualityInspectionService = QualityInspectionService = __decorate([
...
//# sourceMappingURL=quality-inspection.service.js.map