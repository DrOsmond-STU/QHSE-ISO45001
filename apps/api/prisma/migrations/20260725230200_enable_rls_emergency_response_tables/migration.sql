...
DO $$ BEGIN IF EXISTS (SELECT 1 FROM pg_roles WHERE rolname = 'qhse_app') THEN EXECUTE $sql$GRANT SELECT, INSERT, UPDATE, DELETE ON "emergency_muster_points" TO qhse_app$sql$; END IF; END $$;
...
CREATE POLICY tenant_isolation_policy ON "emergency_response_plans"
  USING (tenant_id = current_setting('app.current_tenant_id', true)::uuid);
DO $$ BEGIN IF EXISTS (SELECT 1 FROM pg_roles WHERE rolname = 'qhse_app') THEN EXECUTE $sql$GRANT SELECT, INSERT, UPDATE, DELETE ON "emergency_response_plans" TO qhse_app$sql$; END IF; END $$;
...
CREATE POLICY tenant_isolation_policy ON "emergency_response_plan_steps"
  USING (tenant_id = current_setting('app.current_tenant_id', true)::uuid);
DO $$ BEGIN IF EXISTS (SELECT 1 FROM pg_roles WHERE rolname = 'qhse_app') THEN EXECUTE $sql$GRANT SELECT, INSERT, UPDATE, DELETE ON "emergency_response_plan_steps" TO qhse_app$sql$; END IF; END $$;
...
CREATE POLICY tenant_isolation_policy ON "emergency_response_teams"
  USING (tenant_id = current_setting('app.current_tenant_id', true)::uuid);
DO $$ BEGIN IF EXISTS (SELECT 1 FROM pg_roles WHERE rolname = 'qhse_app') THEN EXECUTE $sql$GRANT SELECT, INSERT, UPDATE, DELETE ON "emergency_response_teams" TO qhse_app$sql$; END IF; END $$;
...
CREATE POLICY tenant_isolation_policy ON "emergency_response_team_members"
  USING (tenant_id = current_setting('app.current_tenant_id', true)::uuid);
DO $$ BEGIN IF EXISTS (SELECT 1 FROM pg_roles WHERE rolname = 'qhse_app') THEN EXECUTE $sql$GRANT SELECT, INSERT, UPDATE, DELETE ON "emergency_response_team_members" TO qhse_app$sql$; END IF; END $$;
...
CREATE POLICY tenant_isolation_policy ON "emergency_contacts"
  USING (tenant_id = current_setting('app.current_tenant_id', true)::uuid);
DO $$ BEGIN IF EXISTS (SELECT 1 FROM pg_roles WHERE rolname = 'qhse_app') THEN EXECUTE $sql$GRANT SELECT, INSERT, UPDATE, DELETE ON "emergency_contacts" TO qhse_app$sql$; END IF; END $$;
...
CREATE POLICY tenant_isolation_policy ON "emergency_drills"
  USING (tenant_id = current_setting('app.current_tenant_id', true)::uuid);
DO $$ BEGIN IF EXISTS (SELECT 1 FROM pg_roles WHERE rolname = 'qhse_app') THEN EXECUTE $sql$GRANT SELECT, INSERT, UPDATE, DELETE ON "emergency_drills" TO qhse_app$sql$; END IF; END $$;
...
CREATE POLICY tenant_isolation_policy ON "drill_participation_logs"
  USING (tenant_id = current_setting('app.current_tenant_id', true)::uuid);
DO $$ BEGIN IF EXISTS (SELECT 1 FROM pg_roles WHERE rolname = 'qhse_app') THEN EXECUTE $sql$GRANT SELECT, INSERT, UPDATE, DELETE ON "drill_participation_logs" TO qhse_app$sql$; END IF; END $$;
...
CREATE POLICY tenant_isolation_policy ON "emergency_activations"
  USING (tenant_id = current_setting('app.current_tenant_id', true)::uuid);
DO $$ BEGIN IF EXISTS (SELECT 1 FROM pg_roles WHERE rolname = 'qhse_app') THEN EXECUTE $sql$GRANT SELECT, INSERT, UPDATE, DELETE ON "emergency_activations" TO qhse_app$sql$; END IF; END $$;
...
CREATE POLICY tenant_isolation_policy ON "muster_point_checkins"
  USING (tenant_id = current_setting('app.current_tenant_id', true)::uuid);
DO $$ BEGIN IF EXISTS (SELECT 1 FROM pg_roles WHERE rolname = 'qhse_app') THEN EXECUTE $sql$GRANT SELECT, INSERT, UPDATE, DELETE ON "muster_point_checkins" TO qhse_app$sql$; END IF; END $$;
...
CREATE POLICY tenant_isolation_policy ON "emergency_equipment_readiness_checklist"
  USING (tenant_id = current_setting('app.current_tenant_id', true)::uuid);
DO $$ BEGIN IF EXISTS (SELECT 1 FROM pg_roles WHERE rolname = 'qhse_app') THEN EXECUTE $sql$GRANT SELECT, INSERT, UPDATE, DELETE ON "emergency_equipment_readiness_checklist" TO qhse_app$sql$; END IF; END $$;
