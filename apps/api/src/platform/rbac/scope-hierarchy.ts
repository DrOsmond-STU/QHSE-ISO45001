...
export const SCOPE_LEVEL_ORDER: Record<ScopeType, number> = {
  TENANT: 0,
  COMPANY: 1,
  BRANCH: 2,
  SITE: 3,
...
function ancestorIdAtLevel(ancestors: ScopeAncestors, level: ScopeType): string | undefined {
  switch (level) {
    case "TENANT":
...
export function scopeCovers(
  assignment: { scopeType: ScopeType; scopeId: string | null },
  target: { scopeType: ScopeType; scopeId: string },
  ...
  if (SCOPE_LEVEL_ORDER[assignment.scopeType] >= SCOPE_LEVEL_ORDER[target.scopeType]) {
