...
GRANT SELECT, INSERT, UPDATE, DELETE ON "risk_matrix_configs" TO qhse_app;
...
CREATE POLICY tenant_isolation_policy ON "risk_matrix_levels"
  USING (tenant_id = current_setting('app.current_tenant_id', true)::uuid);
GRANT SELECT, INSERT, UPDATE, DELETE ON "risk_matrix_levels" TO qhse_app;
...
CREATE POLICY tenant_isolation_policy ON "risk_matrix_cells"
  USING (tenant_id = current_setting('app.current_tenant_id', true)::uuid);
GRANT SELECT, INSERT, UPDATE, DELETE ON "risk_matrix_cells" TO qhse_app;
...
CREATE POLICY tenant_isolation_policy ON "hazard_register"
  USING (tenant_id = current_setting('app.current_tenant_id', true)::uuid);
GRANT SELECT, INSERT, UPDATE, DELETE ON "hazard_register" TO qhse_app;
