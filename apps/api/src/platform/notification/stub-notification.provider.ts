...
export class StubNotificationProvider implements NotificationProvider {
  async sendEmail(_params: SendEmailParams): Promise<NotificationSendResult> {
    throw new Error("Provider email belum dikonfigurasi — menunggu Modul 30 (API Integration).");
  }

  async sendWhatsApp(_params: SendChatMessageParams): Promise<NotificationSendResult> {
    throw new Error("Provider WhatsApp Business API belum dikonfigurasi — menunggu Modul 30 (API Integration).");
  }

  async sendTelegram(_params: SendChatMessageParams): Promise<NotificationSendResult> {
    throw new Error("Provider Telegram Bot API belum dikonfigurasi — menunggu Modul 30 (API Integration).");
  }

  async sendInApp(params: SendInAppParams): Promise<NotificationSendResult> {
