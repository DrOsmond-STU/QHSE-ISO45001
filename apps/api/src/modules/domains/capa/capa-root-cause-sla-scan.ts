...
export function findCapaRootCauseSlaDue(candidates: CapaRootCauseSlaCandidate[], now: Date): CapaRootCauseSlaCandidate[] {
  const windowMs = ROOT_CAUSE_SLA_WARNING_DAYS * 24 * 60 * 60 * 1000;
  return candidates.filter((c) => {
    if (c.status !== "DRAFT" || c.rootCauseSlaReminderSentAt !== null) return false;
