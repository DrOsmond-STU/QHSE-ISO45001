...
DO $$ BEGIN IF EXISTS (SELECT 1 FROM pg_roles WHERE rolname = 'qhse_app') THEN EXECUTE $sql$GRANT SELECT, INSERT, UPDATE, DELETE ON "capa_register" TO qhse_app$sql$; END IF; END $$;
...
CREATE POLICY tenant_isolation_policy ON "capa_root_cause_analysis"
  USING (tenant_id = current_setting('app.current_tenant_id', true)::uuid);
DO $$ BEGIN IF EXISTS (SELECT 1 FROM pg_roles WHERE rolname = 'qhse_app') THEN EXECUTE $sql$GRANT SELECT, INSERT, UPDATE, DELETE ON "capa_root_cause_analysis" TO qhse_app$sql$; END IF; END $$;
...
CREATE POLICY tenant_isolation_policy ON "capa_action_plans"
  USING (tenant_id = current_setting('app.current_tenant_id', true)::uuid);
DO $$ BEGIN IF EXISTS (SELECT 1 FROM pg_roles WHERE rolname = 'qhse_app') THEN EXECUTE $sql$GRANT SELECT, INSERT, UPDATE, DELETE ON "capa_action_plans" TO qhse_app$sql$; END IF; END $$;
...
CREATE POLICY tenant_isolation_policy ON "capa_effectiveness_verification"
  USING (tenant_id = current_setting('app.current_tenant_id', true)::uuid);
DO $$ BEGIN IF EXISTS (SELECT 1 FROM pg_roles WHERE rolname = 'qhse_app') THEN EXECUTE $sql$GRANT SELECT, INSERT, UPDATE, DELETE ON "capa_effectiveness_verification" TO qhse_app$sql$; END IF; END $$;
...
CREATE POLICY tenant_isolation_policy ON "capa_approvals"
  USING (tenant_id = current_setting('app.current_tenant_id', true)::uuid);
DO $$ BEGIN IF EXISTS (SELECT 1 FROM pg_roles WHERE rolname = 'qhse_app') THEN EXECUTE $sql$GRANT SELECT, INSERT, UPDATE, DELETE ON "capa_approvals" TO qhse_app$sql$; END IF; END $$;
