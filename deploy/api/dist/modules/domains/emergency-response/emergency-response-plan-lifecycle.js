...
exports.validateEmergencyResponsePlanStatusTransition = validateEmergencyResponsePlanStatusTransition;
exports.isPlanReviewOverdueBy30Days = isPlanReviewOverdueBy30Days;
exports.computeNextReviewDueDate = computeNextReviewDueDate;
...
function validateEmergencyResponsePlanStatusTransition(from, to) {
...
// tanggal).
function isPlanReviewOverdueBy30Days(nextReviewDueDate, now) {
...
function computeNextReviewDueDate(fromDate, frequency) {
    const next = new Date(fromDate);
    const yearsToAdd = frequency === "BIENNIAL" ? 2 : 1;
    next.setUTCFullYear(next.getUTCFullYear() + yearsToAdd);
    return next;
}
//# sourceMappingURL=emergency-response-plan-lifecycle.js.map