...
export function assertDocumentAcceptsNewDistribution(status: DocumentStatus): void {
...
// ALL_TENANT."
export function assertClassificationAllowsTarget(
  ...
  if (restricted && targetType !== "USER" && targetType !== "ROLE") {
    throw new Error(
      `Dokumen classification=${classification} hanya boleh didistribusikan ke target USER/ROLE, bukan ${targetType} (BR-07).`,
    );
  }
}
