...
// Endpoint HTTP READ-ONLY demo utk domain Asset & Equipment Management
// (Modul 15) — GET list+detail assets, pola sama document.controller.ts
...
let AssetController = class AssetController {
    ...
    async list(query) {
        ...
        return this.prisma.withRls(async (tx) => {
            const [data, total] = await Promise.all([
                tx.asset.findMany({ where: { tenantId }, orderBy: { createdAt: "desc" }, skip: (page - 1) * limit, take: limit }),
    ...
    async getById(id) {
        const record = await this.prisma.withRls((tx) => tx.asset.findUniqueOrThrow({ where: { id } }));
        return { data: record };
    }
};
exports.AssetController = AssetController;
__decorate([
    (0, require_permission_decorator_1.RequirePermission)("asset.asset.read"),
    (0, common_1.Get)(),
