 ...
 */
export function assertClosureReadyForClosed(closureStatus: WorkPermitClosureStatus, requiresLoto: boolean, isolationRemovedConfirmed: boolean): void {
  ...
  if (requiresLoto && !isolationRemovedConfirmed) {
    throw new Error(
      "work_permit_closures.isolation_removed_confirmed wajib TRUE sebelum work_permits dapat berstatus CLOSED kalau tipe mewajibkan LOTO (BR-06).",
    );
  }
}
