...
export class DelegationOverlapError extends Error {}
 ...
 */
export function assertNoDelegationOverlap(
  newRange: DelegationDateRange,
  existingDelegations: readonly DelegationDateRange[],
): void {
  for (const existing of existingDelegations) {
    if (newRange.dateFrom.getTime() <= existing.dateTo.getTime() && existing.dateFrom.getTime() <= newRange.dateTo.getTime()) {
      throw new DelegationOverlapError(
        `BR-07: rentang tanggal ${newRange.dateFrom.toISOString().slice(0, 10)}..${newRange.dateTo.toISOString().slice(0, 10)} tumpang tindih dengan delegasi existing ${existing.dateFrom.toISOString().slice(0, 10)}..${existing.dateTo.toISOString().slice(0, 10)}.`,
      );
    }
  }
}
...
export function findDelegationsToActivate(
  candidates: readonly ScheduledDelegationCandidate[],
  now: Date,
...
export function findDelegationsToExpire(
  candidates: readonly ActiveDelegationCandidate[],
  now: Date,
