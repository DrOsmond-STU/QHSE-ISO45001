...
describe("AppLoggerService", () => {
  ...
  let pinoInfoSpy: jest.SpyInstance;

  beforeEach(() => {
    logger = new AppLoggerService();
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    pinoInfoSpy = jest.spyOn((logger as any).pino, "info").mockImplementation(() => undefined);
  });
  ...
  it("event() menyertakan tenant_id/user_id/correlation_id dari AsyncLocalStorage otomatis", () => {
    tenantContextStorage.run({ tenantId: "tenant-1", userId: "user-1" }, () => {
      observabilityContextStorage.run({ correlationId: "corr-1" }, () => {
  ...
  it("event() di luar request context -> tenant_id/user_id/correlation_id null (bukan undefined/crash)", () => {
    ...
    expect(pinoInfoSpy).toHaveBeenCalledWith(
      expect.objectContaining({ tenant_id: null, user_id: null, correlation_id: null }),
      "job cross-tenant",
    );
  });
  ...
  it("meta tambahan (mis. partition_table) ikut disertakan apa adanya", () => {
    ...
    expect(pinoInfoSpy).toHaveBeenCalledWith(
      expect.objectContaining({ partition_table: "system_audit_logs_y2027m01" }),
      "partisi dibuat",
    );
  });
});
