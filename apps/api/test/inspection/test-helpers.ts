...
/** site_id pada inspection_schedules/inspection_records PUNYA FK sungguhan —
 * test yang perlu mengisi siteId wajib pakai baris nyata, bukan randomUUID().
 * Pola PERSIS test/incident/test-helpers.ts (3.5). */
...
/** BR-02 gate mengecek `attachments` LANGSUNG (bukan lewat AttachmentService,
 * belum ada controller upload dipakai di modul ini) — insert baris minimal
 * scanStatus=CLEAN cukup utk memenuhi gate, storage/putObject sungguhan
 * tidak diperlukan krn complete() tidak pernah membaca isi filenya. */
export async function seedCleanAttachment(tenantId: string, entityType: string, entityId: string, uploadedBy: string): Promise<void> {
  await adminPrisma.attachment.create({
    data: {
      tenantId,
      entityType,
      entityId,
      fileName: "bukti-foto.jpg",
      fileUrl: `test-fixture/${randomUUID()}.jpg`,
      fileSize: 1024,
      mimeType: "image/jpeg",
      scanStatus: "CLEAN",
      uploadedBy,
