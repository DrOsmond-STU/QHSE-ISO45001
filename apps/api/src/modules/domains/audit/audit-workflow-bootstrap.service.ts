...
import { requireTenantId } from "./audit-context";
 ...
 */
@Injectable()
export class AuditWorkflowBootstrapService {
  ...
  private async findRoleOrThrow(tx: Prisma.TransactionClient, roleCode: string) {
    ...
    if (!role) {
      throw new NotFoundException(
        `Role sistem "${roleCode}" tidak ditemukan — RBAC baseline (seed-rbac-baseline.ts) wajib dijalankan sebelum modul Audit Management bisa membuat workflow_definitions default.`,
      );
    }
    return role;
  }
   ...
   */
  async ensureAuditProgramWorkflowDefinition(tx: Prisma.TransactionClient): Promise<WorkflowDefinition> {
    const tenantId = requireTenantId();
   ...
   */
  async ensureAuditReportWorkflowDefinition(tx: Prisma.TransactionClient): Promise<WorkflowDefinition> {
    const tenantId = requireTenantId();
