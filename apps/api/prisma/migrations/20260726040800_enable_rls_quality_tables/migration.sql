...
DO $$ BEGIN IF EXISTS (SELECT 1 FROM pg_roles WHERE rolname = 'qhse_app') THEN EXECUTE $sql$GRANT SELECT, INSERT, UPDATE, DELETE ON "ncr_records" TO qhse_app$sql$; END IF; END $$;
...
CREATE POLICY tenant_isolation_policy ON "customer_complaints"
  USING (tenant_id = current_setting('app.current_tenant_id', true)::uuid);
DO $$ BEGIN IF EXISTS (SELECT 1 FROM pg_roles WHERE rolname = 'qhse_app') THEN EXECUTE $sql$GRANT SELECT, INSERT, UPDATE, DELETE ON "customer_complaints" TO qhse_app$sql$; END IF; END $$;
...
CREATE POLICY tenant_isolation_policy ON "quality_inspections"
  USING (tenant_id = current_setting('app.current_tenant_id', true)::uuid);
DO $$ BEGIN IF EXISTS (SELECT 1 FROM pg_roles WHERE rolname = 'qhse_app') THEN EXECUTE $sql$GRANT SELECT, INSERT, UPDATE, DELETE ON "quality_inspections" TO qhse_app$sql$; END IF; END $$;
...
CREATE POLICY tenant_isolation_policy ON "quality_inspection_parameters"
  USING (tenant_id = current_setting('app.current_tenant_id', true)::uuid);
DO $$ BEGIN IF EXISTS (SELECT 1 FROM pg_roles WHERE rolname = 'qhse_app') THEN EXECUTE $sql$GRANT SELECT, INSERT, UPDATE, DELETE ON "quality_inspection_parameters" TO qhse_app$sql$; END IF; END $$;
...
CREATE POLICY tenant_isolation_policy ON "supplier_quality_records"
  USING (tenant_id = current_setting('app.current_tenant_id', true)::uuid);
DO $$ BEGIN IF EXISTS (SELECT 1 FROM pg_roles WHERE rolname = 'qhse_app') THEN EXECUTE $sql$GRANT SELECT, INSERT, UPDATE, DELETE ON "supplier_quality_records" TO qhse_app$sql$; END IF; END $$;
...
CREATE POLICY tenant_isolation_policy ON "quality_objectives"
  USING (tenant_id = current_setting('app.current_tenant_id', true)::uuid);
DO $$ BEGIN IF EXISTS (SELECT 1 FROM pg_roles WHERE rolname = 'qhse_app') THEN EXECUTE $sql$GRANT SELECT, INSERT, UPDATE, DELETE ON "quality_objectives" TO qhse_app$sql$; END IF; END $$;
...
CREATE POLICY tenant_isolation_policy ON "quality_objective_progress_logs"
  USING (tenant_id = current_setting('app.current_tenant_id', true)::uuid);
DO $$ BEGIN IF EXISTS (SELECT 1 FROM pg_roles WHERE rolname = 'qhse_app') THEN EXECUTE $sql$GRANT SELECT, INSERT, UPDATE, DELETE ON "quality_objective_progress_logs" TO qhse_app$sql$; END IF; END $$;
