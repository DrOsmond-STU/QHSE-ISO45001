  ...
  for (const [categoryKey, assetName, manufacturer, siteKey, safetyCritical] of ASSETS) {
    ...
    assetIdByName[assetName] = await upsert(
      ...
      "asset_id",
      {
        ...
        model_number: `${manufacturer.split(" ")[0].toUpperCase()}-${intBetween(random, 100, 990)}${pick(random, ["A", "B", "X", "S"])}`,
        purchase_date: dateOnly(daysAgo(intBetween(random, 500, 4200))),
  ...
  for (const [name, type, category, risk, status, registeredOffset] of CONTRACTORS) {
    ...
    await upsert(
      ...
      "contractor_id",
      {
        ...
        address: `Jl. Industri Raya No. ${intBetween(random, 5, 199)}`,
        city: pick(random, ["Jakarta Timur", "Cepu", "Balikpapan", "Surabaya", "Bekasi"]),
        province: pick(random, ["DKI Jakarta", "Jawa Tengah", "Kalimantan Timur", "Jawa Timur", "Jawa Barat"]),
        business_registration_no: `AHU-${intBetween(random, 1000000, 9999999)}.AH.01.01.${2010 + (contractorSequence % 12)}`,
        tax_id_npwp: `0${intBetween(random, 1, 9)}.${intBetween(random, 100, 999)}.${intBetween(random, 100, 999)}.${intBetween(random, 1, 9)}-${intBetween(random, 100, 999)}.000`,
        business_license_type: type === "CONSTRUCTION" ? "SBU Konstruksi + IUJP" : type === "ENGINEERING_SERVICES" ? "IUJP Jasa Penunjang Migas" : "NIB + Izin Usaha",
        contact_person_name: pick(random, ["Bapak Suryanto", "Ibu Kartika Dewi", "Bapak Ridwan Hakim", "Ibu Maya Anggraini", "Bapak Teguh Prasetya"]),
        contact_person_phone: `08${intBetween(random, 11, 59)}${intBetween(random, 1000000, 9999999)}`,
        contact_person_email: `hse@${name.toLowerCase().replace(/^(pt|cv)\s+/, "").replace(/\s+/g, "-")}.demo`,
        company_id: ctx.companyId,
      },
      ctx.audit,
    );
