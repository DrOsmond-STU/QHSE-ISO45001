...
export function assertAuditorIndependence(
  candidateDepartmentId: string | null,
  auditeeScopeDepartmentIds: (string | null)[],
): void {
  if (!candidateDepartmentId) return;
  const conflict = auditeeScopeDepartmentIds.includes(candidateDepartmentId);
  if (conflict) {
    throw new Error(
      "audit_team_members tidak dapat ditambahkan — kandidat berasal dari department yang sama dengan salah satu audit_auditee_scopes audit ini (BR-07, independensi auditor).",
    );
  }
}
