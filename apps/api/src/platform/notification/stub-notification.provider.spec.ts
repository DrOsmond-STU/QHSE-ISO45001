...
describe("StubNotificationProvider", () => {
  ...
  it("sendEmail throw eksplisit (belum ada Modul 30) — bukan silent no-op", async () => {
    await expect(provider.sendEmail({ toEmail: "a@b.com", subject: "s", body: "b" })).rejects.toThrow(
      /belum dikonfigurasi/,
    );
  });

  it("sendWhatsApp throw eksplisit (belum ada Modul 30)", async () => {
    await expect(provider.sendWhatsApp({ toAddress: "+6281234567890", body: "b" })).rejects.toThrow(
      /belum dikonfigurasi/,
    );
  });
