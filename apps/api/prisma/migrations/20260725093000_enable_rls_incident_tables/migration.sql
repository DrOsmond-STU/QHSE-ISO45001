...
DO $$ BEGIN IF EXISTS (SELECT 1 FROM pg_roles WHERE rolname = 'qhse_app') THEN EXECUTE $sql$GRANT SELECT, INSERT, UPDATE, DELETE ON "incident_reports" TO qhse_app$sql$; END IF; END $$;
...
CREATE POLICY tenant_isolation_policy ON "incident_investigations"
  USING (tenant_id = current_setting('app.current_tenant_id', true)::uuid);
DO $$ BEGIN IF EXISTS (SELECT 1 FROM pg_roles WHERE rolname = 'qhse_app') THEN EXECUTE $sql$GRANT SELECT, INSERT, UPDATE, DELETE ON "incident_investigations" TO qhse_app$sql$; END IF; END $$;
...
CREATE POLICY tenant_isolation_policy ON "incident_investigation_team"
  USING (tenant_id = current_setting('app.current_tenant_id', true)::uuid);
DO $$ BEGIN IF EXISTS (SELECT 1 FROM pg_roles WHERE rolname = 'qhse_app') THEN EXECUTE $sql$GRANT SELECT, INSERT, UPDATE, DELETE ON "incident_investigation_team" TO qhse_app$sql$; END IF; END $$;
...
CREATE POLICY tenant_isolation_policy ON "incident_root_causes"
  USING (tenant_id = current_setting('app.current_tenant_id', true)::uuid);
DO $$ BEGIN IF EXISTS (SELECT 1 FROM pg_roles WHERE rolname = 'qhse_app') THEN EXECUTE $sql$GRANT SELECT, INSERT, UPDATE, DELETE ON "incident_root_causes" TO qhse_app$sql$; END IF; END $$;
...
CREATE POLICY tenant_isolation_policy ON "incident_corrective_actions"
  USING (tenant_id = current_setting('app.current_tenant_id', true)::uuid);
DO $$ BEGIN IF EXISTS (SELECT 1 FROM pg_roles WHERE rolname = 'qhse_app') THEN EXECUTE $sql$GRANT SELECT, INSERT, UPDATE, DELETE ON "incident_corrective_actions" TO qhse_app$sql$; END IF; END $$;
...
CREATE POLICY tenant_isolation_policy ON "incident_witness_statements"
  USING (tenant_id = current_setting('app.current_tenant_id', true)::uuid);
DO $$ BEGIN IF EXISTS (SELECT 1 FROM pg_roles WHERE rolname = 'qhse_app') THEN EXECUTE $sql$GRANT SELECT, INSERT, UPDATE, DELETE ON "incident_witness_statements" TO qhse_app$sql$; END IF; END $$;
...
CREATE POLICY tenant_isolation_policy ON "incident_regulatory_reports"
  USING (tenant_id = current_setting('app.current_tenant_id', true)::uuid);
DO $$ BEGIN IF EXISTS (SELECT 1 FROM pg_roles WHERE rolname = 'qhse_app') THEN EXECUTE $sql$GRANT SELECT, INSERT, UPDATE, DELETE ON "incident_regulatory_reports" TO qhse_app$sql$; END IF; END $$;
...
CREATE POLICY tenant_isolation_policy ON "incident_statistics_cache"
  USING (tenant_id = current_setting('app.current_tenant_id', true)::uuid);
DO $$ BEGIN IF EXISTS (SELECT 1 FROM pg_roles WHERE rolname = 'qhse_app') THEN EXECUTE $sql$GRANT SELECT, INSERT, UPDATE, DELETE ON "incident_statistics_cache" TO qhse_app$sql$; END IF; END $$;
