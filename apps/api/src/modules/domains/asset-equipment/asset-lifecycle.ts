...
const TERMINAL_LIFECYCLE_STATUSES: AssetLifecycleStatus[] = ["RETIRED", "DISPOSED"];
...
export function isMaintenanceDueSoon(nextDueDate: Date, now: Date): boolean {
  const diffDays = (nextDueDate.getTime() - now.getTime()) / MS_PER_DAY;
  return diffDays >= 0 && diffDays <= DUE_SOON_WINDOW_DAYS;
}

export function isMaintenanceOverdue(nextDueDate: Date, now: Date): boolean {
  return nextDueDate.getTime() < now.getTime();
}

export function calculateDaysOverdue(nextDueDate: Date, now: Date): number {
...
export function shouldDeactivateSchedulesOnLifecycleChange(newStatus: AssetLifecycleStatus): boolean {
...
// false menjadi true").
export function isSafetyCriticalNewlyFlagged(previous: boolean, next: boolean): boolean {
...
export function requiresDisposalWorkflow(newStatus: AssetLifecycleStatus, categoryRequiresDisposalApproval: boolean): boolean {
...
export function isSafetyCriticalOutOfServiceAlertRequired(isSafetyCritical: boolean, conditionStatus: AssetConditionStatus): boolean {
...
// gap TDD §26.
export function calculateNextDueDate(fromDate: Date, intervalType: MaintenanceIntervalType, intervalValue: number): Date | null {
