...
export function findExpiredCompetencyRecords(
  candidates: AuditorCompetencyExpiryCandidate[],
  now: Date,
...
export function findCompetencyRecordsApproachingExpiry(
  candidates: AuditorCompetencyExpiryCandidate[],
  now: Date,
): AuditorCompetencyExpiryCandidate[] {
  const windowMs = EXPIRY_WARNING_WINDOW_DAYS * 24 * 60 * 60 * 1000;
  return candidates.filter((c) => {
    if (c.status !== "ACTIVE" || c.expiryDate === null) return false;
