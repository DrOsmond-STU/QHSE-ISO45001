...
export function computeItemScore(responseType: InspectionResponseType, responseValue: string, weight: number): number | null {
  switch (responseType) {
    case "PASS_FAIL":
      if (responseValue === "PASS") return weight;
      if (responseValue === "FAIL") return 0;
      return null;
    case "YES_NO":
      if (responseValue === "YES") return weight;
      if (responseValue === "NO") return 0;
      return null;
    case "SCALE_1_5": {
...
export function computeOverallScore(items: OverallScoreItem[]): number | null {
  const scoreable = items.filter((i): i is { scoreObtained: number; weight: number } => i.scoreObtained !== null);
...
export function computeAllMandatoryItemsPassed(items: MandatoryPassCheckItem[]): boolean {
  return items.every((item) => {
    if (!item.isMandatory) return true;
    if (item.responseType === "PASS_FAIL") return item.responseValue === "PASS";
    if (item.responseType === "YES_NO") return item.responseValue === "YES";
    return true;
  });
}
...
export function computeOverallResult(
  ...
  if (scoringMethod === "CHECKLIST_NO_SCORE") return "NA";
  if (scoringMethod === "PASS_FAIL_ONLY") return allMandatoryPassed ? "PASS" : "FAIL";
  if (overallScore === null || passingScoreThreshold === null) return "NA";
