...
import { CALIBRATION_PROVIDER_ACCREDITATION_WARNING_WINDOW_DAYS, CALIBRATION_REMINDER_TIERS_DAYS } from "./calibration-due-scan.constants";
import { isCalibrationScheduleOverdue, isWithinReminderWindow } from "./calibration-lifecycle";

type ReminderTierColumn = "reminderSentAtDay30" | "reminderSentAtDay14" | "reminderSentAtDay7" | "reminderSentAtDay1";

const TIER_COLUMN_BY_DAYS: Record<number, ReminderTierColumn> = {
 ...
 */
@Injectable()
export class CalibrationDueScanService implements OnModuleDestroy {
  ...
  private async scanSchedules(tenantId: string, now: Date): Promise<PendingNotification[]> {
    return tenantContextStorage.run({ tenantId }, () =>
      this.prisma.withRls(async (tx) => {
        ...
        for (const s of schedules) {
          ...
          if (s.overdueNotifiedAt === null && isCalibrationScheduleOverdue(s.dueDate, now)) {
          ...
          for (const tierDays of CALIBRATION_REMINDER_TIERS_DAYS) {
            const column = TIER_COLUMN_BY_DAYS[tierDays];
            if (s[column] !== null) continue;
            if (!isWithinReminderWindow(s.dueDate, now, tierDays)) continue;
  ...
  private async scanProviders(tenantId: string, now: Date): Promise<PendingNotification[]> {
    return tenantContextStorage.run({ tenantId }, () =>
      this.prisma.withRls(async (tx) => {
        ...
        const dueSoon = providers.filter(
          (p) => p.accreditationValidUntil && isWithinReminderWindow(p.accreditationValidUntil, now, CALIBRATION_PROVIDER_ACCREDITATION_WARNING_WINDOW_DAYS),
        );
